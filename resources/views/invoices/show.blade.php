...
<div class="space-y-8">
    <div>
        <a href="{{ route('invoices.index') }}" class="text-sm font-medium text-blue-600 hover:text-blue-700">← Back to invoices</a>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-8">
        ...
        <div class="flex items-start justify-between pb-8 border-b border-slate-200">
            <div>
                ...
                <p class="text-slate-600 mt-1">Invoice Date: {{ $invoice->created_at->format('M d, Y') }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 py-8 border-b border-slate-200">
            <div>
                ...
                @if ($invoice->user->company)
                    <p class="text-slate-600">{{ $invoice->user->company }}</p>
                @endif
            </div>
            <div class="md:text-right">
                <div class="mb-4">
                    <p class="text-xs font-semibold text-slate-600 uppercase mb-1">Due Date</p>
                    <p class="text-lg font-semibold text-slate-900">{{ $invoice->due_date->format('M d, Y') }}</p>
                </div>
                @if ($invoice->paid_date)
                    <div>
                        <p class="text-xs font-semibold text-slate-600 uppercase mb-1">Paid On</p>
                        <p class="text-lg font-semibold text-emerald-600">{{ $invoice->paid_date->format('M d, Y') }}</p>
                    </div>
                @endif
            </div>
        </div>
