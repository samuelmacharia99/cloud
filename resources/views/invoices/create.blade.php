...
<div class="max-w-2xl space-y-8">
    ...
    <form action="{{ route('invoices.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 p-8 space-y-6">
        @csrf
        ...
        <div>
            ...
            @error('invoice_number') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
        </div>
        ...
        <div>
            ...
            @error('subtotal') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
        </div>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('invoices.index') }}" class="px-6 py-2.5 rounded-lg border border-slate-300 text-slate-700 font-medium hover:bg-slate-50 transition-colors">
                Cancel
            </a>
        </div>
    </form>
