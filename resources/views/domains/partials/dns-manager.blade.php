@php
    $dnsRoutePrefix = $dnsRoutePrefix ?? 'customer.domains.dns';
    $domainShowUrl = $domainShowUrl ?? route('customer.domains.show', $domain);
    $nameserverUrl = $nameserverUrl ?? route('customer.domains.dns.nameservers', $domain);
    $primaryButtonClass = $primaryButtonClass ?? 'bg-blue-600 hover:bg-blue-700';
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        @unless($usesDirectAdmin ?? false)
            <a href="{{ $nameserverUrl }}" class="px-6 py-3 {{ $primaryButtonClass }} text-white font-semibold rounded-lg transition inline-block text-center">
                View Nameservers
            </a>
        @endunless
    </div>
    ...
    @if($usesDirectAdmin ?? false)
        <div class="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6">
            ...
            <p class="text-sm text-blue-800 dark:text-blue-300">This domain is linked to a DirectAdmin hosting account. Manage DNS records from the hosting control panel instead.</p>
        </div>
                            ...
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                                @foreach($records as $record)
                                    ...
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/60 align-top" x-show="editingId !== '{{ $recordId }}'">
                                        ...
                                        <td class="px-4 py-3 text-sm">
                                            @if($canProxy)
                                                <form action="{{ route($dnsRoutePrefix.'.update-record', [$domain, $recordId]) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                        ...
                                        <td class="px-4 py-3 text-sm text-right whitespace-nowrap">
                                            ...
                                            <form action="{{ route($dnsRoutePrefix.'.delete-record', [$domain, $recordId]) }}" method="POST" class="inline" onsubmit="return confirm('Delete this DNS record?');">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <tr class="bg-slate-50 dark:bg-slate-800/40" x-show="editingId === '{{ $recordId }}'" x-cloak>
                                        <td colspan="6" class="px-4 py-4">
                                            <form
                                                action="{{ route($dnsRoutePrefix.'.update-record', [$domain, $recordId]) }}"
                                                method="POST"
                                            ...
                                            >
                                                ...
                                                <div class="md:col-span-2 flex items-center gap-3">
                                                    <button type="submit" class="px-4 py-2 {{ $primaryButtonClass }} text-white text-sm font-medium rounded-lg">Save changes</button>
    ...
    @elseif(($canProvision ?? false) && ($cloudflareAvailable ?? false))
        <div class="ui-card p-8 text-center space-y-4">
            ...
            <form action="{{ route($dnsRoutePrefix.'.provision', $domain) }}" method="POST">
                @csrf
                <button type="submit" class="px-6 py-3 {{ $primaryButtonClass }} text-white font-semibold rounded-lg transition">Enable DNS Management</button>
            </form>
        </div>
    @else
    ...
    <div class="ui-card p-6">
        <h3 class="font-bold text-slate-900 dark:text-white mb-1">
            {{ $domain->cloudflare_dns_enabled ? 'Cloudflare nameservers' : 'Registry nameservers' }}
        </h3>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-3">
            @if($domain->cloudflare_dns_enabled)
                Assigned by Cloudflare for this zone. We publish them at the registry when we can. If Cloudflare still waits, set these on the
                <a href="{{ $domainShowUrl }}" class="text-blue-600 dark:text-blue-400 hover:underline">domain overview</a>.
            @else
                Published at the domain registry. To change them, use the
                <a href="{{ $domainShowUrl }}" class="text-blue-600 dark:text-blue-400 hover:underline">domain overview page</a>.
            @endif
        </p>
