        ...
        <div class="min-h-screen flex">
            ...
            <aside class="app-sidebar w-64 max-w-[85vw] overflow-y-auto flex flex-col fixed inset-y-0 left-0 z-50 h-screen transform transition-transform duration-200 ease-out -translate-x-full md:translate-x-0 md:static md:z-auto md:flex-shrink-0" :class="{ 'translate-x-0': sidebarOpen }">
                ...
                <nav class="flex-1 px-3 py-6 space-y-8" @click="if (window.innerWidth < 768) sidebarOpen = false">
                    <!-- Dashboard -->
                    <div class="space-y-1">
                        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('dashboard') ? 'nav-link-active' : 'nav-link' }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('customer.services.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.services.*', 'customer.services.container.*', 'customer.projects.*') && ! request()->routeIs('customer.services.email.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('customer.catalog.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('customer.catalog.*') ? 'nav-link-active' : 'nav-link-accent' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('customer.domains.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.domains.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.658 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/>
                            </svg>
                            <span class="text-sm font-medium">Domains</span>
                        </a>
                        <a href="{{ route('customer.servers.index', ['type' => 'vps']) }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.servers.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/>
                            </svg>
                            <span class="text-sm font-medium">Buy Servers</span>
                        </a>
                    </div>

                    <!-- Email -->
                    <div class="space-y-2">
                        <p class="nav-section-label">Email</p>
                        <a href="{{ route('customer.email.inboxes') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.email.inboxes', 'customer.services.email.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                            </svg>
                            <span class="text-sm font-medium">Inboxes</span>
                        </a>
                        <a href="{{ route('customer.email-hosting') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.email-hosting') ? 'nav-link-active' : 'nav-link' }}">
                            ...
                            <span class="text-sm font-medium">Order plans</span>
                        </a>
                    </div>

                    <!-- Billing -->
                    <div class="space-y-2">
                        <p class="nav-section-label">Billing</p>
                        <a href="{{ route('customer.invoices.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.invoices.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                            </svg>
                            <span class="text-sm font-medium">Invoices</span>
                        </a>
                        <a href="{{ route('customer.payments.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.payments.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <span class="text-sm font-medium">Payments</span>
                        </a>
                        <a href="{{ route('customer.credits.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.credits.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('profile.edit') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('profile.security') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('profile.security') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                            </svg>
                            <span class="text-sm font-medium">Security</span>
                        </a>
                        <a href="{{ route('profile.notifications') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('profile.notifications*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('customer.tickets.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all {{ request()->routeIs('customer.tickets.*') ? 'nav-link-active' : 'nav-link' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <header class="app-header h-16 flex items-center px-4 sm:px-6 min-w-0">
                    ...
                    <div class="flex items-center gap-2 sm:gap-4 ml-auto shrink-0">
                        ...
                        <a href="{{ route('customer.cart.index') }}" class="relative p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition">
                            ...
                            @php
                                $cartItems = \App\Support\SessionCart::portal();
                                $cartCount = count($cartItems);
                            @endphp
                            @if($cartCount > 0)
                                <span class="absolute -top-1 -right-1 bg-red-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{{ $cartCount }}</span>
                            @endif
                        </a>
