...
<div class="flex items-center gap-2 text-sm">
    ...
    <a href="{{ route('admin.invoices.show', $invoice) }}" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">#{{ str_pad($invoice->id, 5, '0', STR_PAD_LEFT) }}</a>
    <span class="text-slate-400 dark:text-slate-600">/</span>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8">
        <form method="POST" action="{{ route('admin.invoices.update', $invoice) }}" class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select id="user_id" name="user_id" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('user_id') border-red-500 @enderror" required>
                            ...
                            @foreach ($customers as $customer)
                                <option value="{{ $customer->id }}" @selected(old('user_id', $invoice->user_id) == $customer->id)>{{ $customer->name }} ({{ $customer->email }})</option>
                            @endforeach
                        </select>
                        @error('user_id')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="due_at" name="due_at" value="{{ old('due_at', $invoice->due_at?->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('due_at') border-red-500 @enderror">
                        @error('due_at')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-slate-200 dark:border-slate-800">
                <a href="{{ route('admin.invoices.show', $invoice) }}" class="px-6 py-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-medium transition">
                    Cancel
                </a>
