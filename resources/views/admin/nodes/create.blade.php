        ...
        <form method="POST" action="{{ route('admin.nodes.store') }}" class="space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. US-East-01" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('name') border-red-500 @enderror" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('type') border-red-500 @enderror" required>
                            ...
                            <option value="container_host" @selected(old('type') === 'container_host')>Container Host</option>
                            <option value="load_balancer" @selected(old('type') === 'load_balancer')>Load Balancer</option>
                            <option value="database_server" @selected(old('type') === 'database_server')>Database Server</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" id="hostname" name="hostname" value="{{ old('hostname') }}" placeholder="server01.internal" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('hostname') border-red-500 @enderror" required>
                        @error('hostname')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('status') border-red-500 @enderror" required>
                    ...
                    <option value="degraded" @selected(old('status') === 'degraded')>Degraded</option>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="verify_ssl" value="1" class="w-4 h-4 text-blue-600 rounded" @checked(old('verify_ssl') === '1' || old('verify_ssl') === true)>
