...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8">
        <form method="POST" action="{{ route('admin.products.update', $product) }}" class="space-y-8" x-data="{ productType: '{{ old('type', $product->type) }}' }" @change="productType = document.getElementById('type').value">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="type" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('type') border-red-500 @enderror" required @change="productType = $el.value" disabled>
                            ...
                            <option value="container_hosting" @selected(old('type', $product->type) === 'container_hosting')>Application Hosting</option>
                    ...
                    <div x-show="productType === 'shared_hosting'">
                        ...
                        <select id="direct_admin_package_id" name="direct_admin_package_id" class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white text-sm @error('direct_admin_package_id') border-red-500 @enderror">
                            ...
                            @foreach(\App\Models\DirectAdminPackage::where('is_active', true)->orderBy('disk_quota')->get() as $package)
                                <option value="{{ $package->id }}" @selected(old('direct_admin_package_id', $product->direct_admin_package_id) == $package->id)>{{ $package->name }} ({{ $package->disk_quota }}GB disk, {{ $package->num_domains }} domains)</option>
                            @endforeach
                        </select>
