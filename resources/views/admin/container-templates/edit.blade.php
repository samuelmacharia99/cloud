    ...
    <form method="POST" action="{{ route('admin.container-templates.update', $containerTemplate) }}" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <input type="number" name="health_check_timeout_seconds" value="{{ old('health_check_timeout_seconds', $containerTemplate->health_check_timeout_seconds ?? 120) }}" min="30" max="900" class="w-full px-3 py-2 border rounded-lg @error('health_check_timeout_seconds') border-red-500 @enderror" required>
                @error('health_check_timeout_seconds')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="flex items-end">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="strict_health_check" value="1" {{ old('strict_health_check', $containerTemplate->strict_health_check ?? true) ? 'checked' : '' }} class="mr-2">
