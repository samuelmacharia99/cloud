...
<div class="flex-1 overflow-auto">
    ...
    <div class="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 py-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                <a href="{{ route('admin.cron.index') }}" class="text-sm text-blue-600 dark:text-blue-400 hover:underline mb-2 inline-block">← Back to Cron Jobs</a>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $job->name }}</h1>
                <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                    <code class="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-xs font-mono">{{ $job->command }}</code>
                </p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <form action="{{ route('admin.cron.toggle', $job) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="px-4 py-2 {{ $job->enabled ? 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-800' : 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 hover:bg-green-200 dark:hover:bg-green-800' }} rounded-lg font-medium transition-all">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2">{{ $stats['total_runs'] }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-green-600 dark:text-green-400 mt-2">{{ $stats['success_rate'] }}%</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2">
                            ...
                            @else
                                {{ number_format($stats['avg_duration'] / 1000, 2) }}s
                            @endif
                        </p>
                    </div>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @forelse ($logs as $log)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
                                <td class="px-6 py-4">
                                    <span class="text-slate-900 dark:text-slate-100">{{ $log->started_at->format('M d, Y H:i:s') }}</span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @if ($log->output)
                                        <p class="text-slate-900 dark:text-slate-100 truncate max-w-xs">{{ Str::limit($log->output, 50) }}</p>
                                    @else
                                        <span class="text-slate-600 dark:text-slate-400">No output</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <button onclick="expandRow({{ $loop->index }})" class="text-blue-600 dark:text-blue-400 hover:underline text-xs font-medium">
                                        View
                                    </button>
                                </td>
                            </tr>
                            <!-- Expandable Row -->
                            <tr id="expand-{{ $loop->index }}" class="bg-slate-50 dark:bg-slate-800 hidden">
                                <td colspan="5" class="px-6 py-6">
                                    @if ($log->output)
                                        <div class="mb-6">
                                            <h4 class="text-sm font-semibold text-slate-900 dark:text-white mb-3">Output</h4>
                                            <pre class="bg-slate-950 text-emerald-400 p-4 rounded overflow-x-auto text-xs font-mono">{{ $log->output }}</pre>
                                        </div>
                                    @endif
                                    ...
                                    @if ($log->exception)
                                        <div>
                                            ...
                                            <pre class="bg-red-950 text-red-300 p-4 rounded overflow-x-auto text-xs font-mono">{{ $log->exception }}</pre>
                                        </div>
                                    @endif

                                    @if (!$log->output && !$log->exception)
...
<script>
    ...
    const logsData = @json($logs->map(fn($log) => [
    ...
    setInterval(function() {
        fetch('{{ route("admin.cron.logs", $job) }}')
            .then(response => response.json())
            .catch(e => {});
    }, 15000);
</script>
