@foreach($usageAppItems as $item)
    @php
        $key = $item['key'];
        $linkedDomain = $linkedAppDomains[$key] ?? null;
    ...
    <div class="border-t border-slate-200 dark:border-slate-700 pt-6 first:border-t-0 first:pt-0"
        ...
        @if(!empty($item['email_product']))
            <p class="text-xs text-emerald-700 dark:text-emerald-300 mb-3">Email ({{ $item['email_product']->name }}) is included for this domain.</p>
        @endif

        @if($linkedDomain)
            <input type="hidden" name="app_domain_mode[{{ $key }}]" value="from_cart">
        ...
        @elseif(! empty($item['mail_domain']))
            <input type="hidden" name="app_domain_mode[{{ $key }}]" value="existing">
            <input type="hidden" name="app_domain_fqdn[{{ $key }}]" value="{{ $item['mail_domain'] }}">
            <div class="rounded-lg bg-teal-50 dark:bg-teal-950/30 border border-teal-200 dark:border-teal-800 p-4 mb-4">
                <p class="font-medium text-teal-900 dark:text-teal-100">Domain from your website</p>
        ...
        @else
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-4">Choose how you want to connect a domain to this application. The same domain is used for email when included.</p>

            <input type="hidden" name="app_domain_mode[{{ $key }}]" x-model="mode">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
                <button type="button" @click="mode = 'register'; clearDomainCart()"
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Pay only for the domain. Hosting may be free at first.</p>
                </button>
                <button type="button" @click="mode = 'existing'; clearDomainCart()"
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Point DNS to Talksasa after checkout.</p>
                </button>
            ...
            <div x-show="mode === 'register'" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <input type="text" name="app_domain_name[{{ $key }}]" x-model="domain" @input="resetAvailability()" @blur="parseDomainInput()" placeholder="example or example.com" :disabled="mode !== 'register'"
                    ...
                    <div>
                        ...
                        <select name="app_domain_extension[{{ $key }}]" x-model="extension" @change="resetAvailability()" :disabled="mode !== 'register'"
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                    ...
                    <div>
                        ...
                        <select name="app_domain_years[{{ $key }}]" x-model="years" @change="onYearsChange()" :disabled="mode !== 'register'"
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                            @for($y = 1; $y <= 5; $y++)
                ...
                <input type="hidden" name="app_domain_added[{{ $key }}]" :value="addedToCart ? '1' : '0'">
                <p class="text-xs text-amber-700 dark:text-amber-300">Registration fees are added to your order total. Hosting may be free for your first period.</p>
            </div>
            ...
            <div x-show="mode === 'existing'" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="app_domain_fqdn[{{ $key }}]" x-model="existingFqdn" placeholder="example.com" :disabled="mode !== 'existing'"
                        class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                </div>
                <p
                ...
                >
                    This domain uses Talksasa Cloudflare DNS. App DNS and mail records can be applied automatically after payment.
                </p>
                <div x-show="existingFqdn && !isCloudflareManaged" x-cloak class="rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 p-4 text-sm text-slate-700 dark:text-slate-300">
                    <p class="font-semibold text-blue-900 dark:text-blue-100 mb-2">DNS for your app</p>
                    <p class="text-xs text-slate-600 dark:text-slate-400 mb-3">Point nameservers to Talksasa, or add an A record to your app IP after deploy. Email DNS (MX/SPF/DKIM) is applied when mail is included.</p>
            ...
            <div x-show="mode === 'transfer'" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Domain name</label>
                        <input type="text" name="app_domain_name[{{ $key }}]" x-model="domain" @blur="parseDomainInput()" placeholder="example or example.com" :disabled="mode !== 'transfer'"
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Extension</label>
                        <select name="app_domain_extension[{{ $key }}]" x-model="extension" :disabled="mode !== 'transfer'"
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                            @foreach($domainExtensions as $ext)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="app_transfer_epp[{{ $key }}]" :disabled="mode !== 'transfer'"
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Current registrar</label>
                        <input type="text" name="app_transfer_registrar[{{ $key }}]" :disabled="mode !== 'transfer'"
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Registrar website (optional)</label>
                    <input type="url" name="app_transfer_registrar_url[{{ $key }}]" placeholder="https://" :disabled="mode !== 'transfer'"
                        class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white disabled:opacity-50">
                </div>
                <p class="text-xs text-amber-700 dark:text-amber-300">Transfer fees are added to your invoice. App and email go live once the transfer completes.</p>
            </div>
        @endif

        @error("app_domain_mode.{$key}")
            <p class="text-red-600 dark:text-red-400 text-sm mt-2">{{ $message }}</p>
        @enderror
        @error("app_domain_added.{$key}")
            <p class="text-red-600 dark:text-red-400 text-sm mt-2">{{ $message }}</p>
        @enderror
        @error("app_domain_fqdn.{$key}")
            <p class="text-red-600 dark:text-red-400 text-sm mt-2">{{ $message }}</p>
        @enderror
    </div>
@endforeach
...
<script>
function usageAppDomainConfig(cartKey, defaultNs, allowedExtensions, linkedDomain, ownedDomains, cfDomains) {
    return {
        cartKey,
