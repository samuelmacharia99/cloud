<section class="space-y-6">
    ...
    <div class="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50/60 dark:bg-amber-950/20 p-6 space-y-3">
        ...
        <ul class="text-sm text-amber-900/90 dark:text-amber-100/90 space-y-2">
            ...
            <li><strong>Uploads sit on a spinner and never finish</strong> — The request is stalling at the web proxy, so nothing reaches WordPress and the container log stays silent. Run <strong>Container Doctor</strong> and use <em>Refresh web proxy</em>, then retry.</li>
            <li><strong>Media Library shows grey document icons</strong> — The uploads saved, but WordPress never built their thumbnails. Run <strong>Container Doctor</strong>: it reports whether PHP has a working image library and offers <em>Repair image processing</em> or <em>Rebuild thumbnails</em>. No re-upload is needed.</li>
            <li><strong>Images load on the dashboard but not on the site</strong> — WordPress is still building URLs from an old address. Container Doctor's <em>Fix site URLs</em> updates <code class="font-mono text-xs">home</code>/<code class="font-mono text-xs">siteurl</code> and rewrites stored links, leaving GUIDs untouched.</li>
