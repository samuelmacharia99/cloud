<section class="space-y-6">
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-3">
            ...
            <ul class="text-sm text-slate-600 dark:text-slate-300 space-y-2">
                ...
                <li>For Next.js/Nuxt apps, Talksasa prefers <code class="font-mono text-xs">npm ci --include=dev --legacy-peer-deps</code> (falls back to <code class="font-mono text-xs">npm install --include=dev --legacy-peer-deps</code> if the lockfile is stale or peers conflict) → prepare (including <code class="font-mono text-xs">prisma generate</code> when a schema exists) → <code class="font-mono text-xs">node node_modules/next/dist/bin/next build</code> (Next) / <code class="font-mono text-xs">npm run build</code> after each Git pull when a framework build is detected. Alpine builds install OpenSSL first so Prisma can load the linux-musl-openssl-3.0.x engine.</li>
    ...
    <div class="rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-3">
        ...
        <ul class="text-sm text-slate-600 dark:text-slate-300 space-y-2">
            <li>Many Vite starters use <code class="font-mono text-xs">"start": "tsx server.ts"</code> or <code class="font-mono text-xs">"start": "node dist/server.cjs"</code> with a custom server that serves the SPA and <code class="font-mono text-xs">/api</code> routes. That fails if production installs strip <code class="font-mono text-xs">vite</code>.</li>
            <li>Talksasa keeps that start command so API routes keep working, runs the build when needed, and installs the full dependency tree (including Vite) for these apps.</li>
            <li>Bare <code class="font-mono text-xs">vite</code> CLI starts (SPA only, no custom server) are rewritten to <code class="font-mono text-xs">vite preview --host 0.0.0.0 --port $PORT --strictPort</code>.</li>
            <li>Vite only answers for hostnames on its allowlist when using preview. Bound domains are passed automatically.</li>
            ...
            <li>If an older deploy is crash-looping on <code class="font-mono text-xs">Cannot find package/module 'vite'</code>, use Container Doctor → <strong>Repair Vite runtime</strong>.</li>
        </ul>
    </div>
    ...
    <div class="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50/60 dark:bg-amber-950/20 p-6 space-y-3">
        ...
        <ul class="text-sm text-amber-900/90 dark:text-amber-100/90 space-y-2">
            ...
            <li><strong>Cannot find package/module 'vite'</strong> — The start script still needs Vite at runtime. Use Container Doctor → <strong>Repair Vite runtime</strong> (keeps your server and <code class="font-mono text-xs">/api</code> routes).</li>
