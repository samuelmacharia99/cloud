...
@section('title', 'Domain Renewal Checkout')
...
<div class="flex items-center gap-2">
    ...
    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Renewal Checkout</p>
</div>
@endsection
...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Header -->
    <div>
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Domain Renewal Checkout</h1>
        <p class="text-slate-600 dark:text-slate-400 mt-2">Review and confirm your domain renewal order</p>
    </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        ...
                        <p class="font-semibold text-slate-900 dark:text-white">Domain Renewal</p>
                    </div>

                    <!-- Renewal Period -->
                    <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">Renewal Period</p>
                        <p class="font-semibold text-slate-900 dark:text-white">{{ $years }} Year{{ $years > 1 ? 's' : '' }}</p>
                    </div>

                    <!-- Current Expiry -->
                    <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">Current Expiry</p>
                        <p class="font-semibold text-slate-900 dark:text-white">{{ $domain->expires_at?->format('M d, Y') ?? 'N/A' }}</p>
                    </div>

                    <!-- Status -->
                    <div class="p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                        <p class="text-sm text-blue-900 dark:text-blue-300">
                            <strong>Status:</strong> After payment, your domain will be renewed for {{ $years }} additional year{{ $years > 1 ? 's' : '' }}.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <form action="{{ route('customer.domains.renewal-checkout-confirm') }}" method="POST" class="space-y-6">
                    ...
                    <div class="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                        ...
                        <label for="agree_terms" class="text-sm text-slate-700 dark:text-slate-300">
                            I agree to the terms and conditions. I understand that after payment, this domain will be renewed for {{ $years }} additional year{{ $years > 1 ? 's' : '' }}.
                        </label>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <a href="{{ route('customer.domains.index') }}" class="flex-1 px-6 py-3 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition text-center">
                            Back
                        </a>
                        <button
                            type="submit"
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 sticky top-6 space-y-4">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600 dark:text-slate-400">Renewal Fee ({{ $years }} year{{ $years > 1 ? 's' : '' }})</span>
                ...
                <div class="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg space-y-2 text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <p>✓ Instant domain renewal</p>
                    <p>✓ Automatic admin notification</p>
                </div>
