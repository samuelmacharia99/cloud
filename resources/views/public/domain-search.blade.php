...
<head>
    ...
    <title>Domain Search Results — {{ config('app.name', 'Talksasa Cloud') }}</title>
    ...
    <section class="pt-32 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-12 text-center">
                ...
                <p class="text-xl text-gray-600 mb-8">Showing results for <span class="font-semibold text-blue-600">"{{ $searchQuery }}"</span></p>

                <!-- New Search Form -->
                <form action="{{ route('domains.search.public') }}" method="GET" class="flex gap-2 max-w-2xl mx-auto mb-8">
                    ...
                    @if (!empty($availableDomains))
                        <div class="mb-8">
                            <h2 class="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                                ...
                                Available Domains ({{ count($availableDomains) }})
                            </h2>

                            <div class="space-y-3">
                                @foreach ($availableDomains as $result)
                                    <div class="bg-white rounded-xl border border-green-200 p-6 hover:shadow-lg transition">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="text-right">
                                                <p class="text-3xl font-bold text-gray-900">
                                                    KES {{ number_format($result['price'], 0) }}
                                                </p>
                                        ...
                                        @auth
                                            <div class="mt-4 pt-4 border-t border-gray-200 flex gap-3">
                                                <form action="{{ route('customer.cart.add') }}" method="POST" style="display:inline;" class="flex-1">
                                                    @csrf
                                                    <input type="hidden" name="item_type" value="domain">
                                                    <input type="hidden" name="item_name" value="{{ $result['full_domain'] }}">
                                                    <input type="hidden" name="item_price" value="{{ $result['price'] }}">
                                        ...
                                        @else
                                            <div class="mt-4 pt-4 border-t border-gray-200">
                                                <a href="{{ route('register') }}?domain={{ $result['full_domain'] }}" class="inline-block w-full text-center px-4 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg font-semibold hover:shadow-lg transition">
                                                    Register Now
                                                </a>
                                            </div>
                                        @endauth
                    ...
                    @if (!empty($unavailableDomains))
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                                ...
                                Unavailable Domains ({{ count($unavailableDomains) }})
                            </h2>

                            <div class="space-y-3">
                                @foreach ($unavailableDomains as $result)
