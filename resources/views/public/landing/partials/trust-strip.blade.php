@if ($landing['show_trust'] ?? true)
<section class="{{ $trustClass ?? 'bg-white border-b border-slate-200' }}">
    <div class="{{ $trustContainerClass ?? 'max-w-6xl mx-auto px-4 py-4' }}">
        <div class="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs sm:text-sm {{ $trustTextClass ?? 'text-slate-600' }}">
