...
<div class="max-w-2xl space-y-8">
    ...
    <form action="{{ route('services.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 p-8 space-y-6">
        @csrf
        ...
        <div>
            ...
            <select name="product_id" required class="w-full px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                @foreach ($products as $product)
                    <option value="{{ $product->id }}">{{ $product->name }} - KSH {{ number_format($product->priceForBillingCycle($product->billing_cycle ?? 'monthly'), 2) }}/{{ $product->billing_cycle }}</option>
                @endforeach
            </select>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('services.index') }}" class="px-6 py-2.5 rounded-lg border border-slate-300 text-slate-700 font-medium hover:bg-slate-50 transition-colors">
                Cancel
            </a>
        </div>
    </form>
