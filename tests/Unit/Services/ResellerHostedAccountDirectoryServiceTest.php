...
class ResellerHostedAccountDirectoryServiceTest extends TestCase
{
    ...
    public function test_reseller_customers_page_lists_portal_customers_only_not_directadmin(): void
    {
        ...
        $portalCustomer = User::factory()->customer()->create([
        ...
        $this->assertTrue(
            collect($result['rows']->items())->contains(fn ($row) => $row instanceof User && $row->is($portalCustomer))
        );
    }
