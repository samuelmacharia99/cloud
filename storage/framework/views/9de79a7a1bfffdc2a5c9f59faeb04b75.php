        ...
        <div class="lg:col-span-2 space-y-6">
                                        ...
                                        <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                            <?php if(isset($item['billing_cycle'])): ?>
                                                <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                                                    <?php echo e(ucfirst($item['billing_cycle'])); ?>

                                                </span>
                                            <?php else: ?>
                                                <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                                                    <?php echo e($item['years'] ?? 1); ?> Year(s)
                                                </span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <p class="font-medium text-slate-900 dark:text-white">Ksh <?php echo e(number_format($item['amount'], 0)); ?></p>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <form action="<?php echo e(route('customer.cart.remove', $item['key'])); ?>" method="POST" style="display: inline;">
                                                <?php echo csrf_field(); ?>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <select x-model="extension" class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm" required>
                                    ...
                                    <?php $__currentLoopData = $domainExtensions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ext): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($ext->extension); ?>"><?php echo e($ext->extension); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                        ...
                        <div x-show="checked && domain && extension" class="p-4 rounded-lg" :class="available ? 'bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700'">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <form x-show="available" action="<?php echo e(route('customer.cart.add')); ?>" method="POST" class="flex-shrink-0">
                                    <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-12 text-center">
                    ...
                    <a href="<?php echo e(route('customer.deploy-service')); ?>" class="inline-flex items-center px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition">
                        Continue Shopping
                    </a>
                </div>
            <?php endif; ?>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 sticky top-4">
                    ...
                    <div class="flex justify-between mb-6">
                        ...
                        <span class="text-2xl font-bold text-blue-600 dark:text-blue-400">Ksh <?php echo e(number_format($total, 0)); ?></span>
                    </div>

                    <a href="<?php echo e(route('customer.checkout.show')); ?>" class="block w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-center transition mb-3">
                        Proceed to Checkout
                    </a>

                    <form action="<?php echo e(route('customer.cart.clear')); ?>" method="POST">
                        <?php echo csrf_field(); ?>
...
function domainChecker() {
    return {
        ...
        async checkAvailability() {
            ...
            try {
                const response = await fetch('<?php echo e(route("customer.cart.check-domain")); ?>', {
                    method: 'POST',
                    headers: {
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/zumi/php/road-map/talksasa-cloud/resources/views/customer/cart/index.blade.php ENDPATH**/ ?>