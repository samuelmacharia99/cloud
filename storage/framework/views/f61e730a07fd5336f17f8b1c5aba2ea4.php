<?php $__env->startSection('title', 'My Domains'); ?>
                        ...
                        <?php $__currentLoopData = $domains; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $domain): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                <td class="px-6 py-4 text-sm font-medium text-slate-900 dark:text-white">
                                    <?php echo e($domain->name); ?><?php echo e($domain->extension); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                    <?php echo e($domain->registered_at?->format('M d, Y') ?? 'N/A'); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                    <?php echo e($domain->expires_at?->format('M d, Y') ?? 'N/A'); ?>

                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium <?php echo e($domain->status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300'); ?>">
                ...
                <input
                    ...
                    @keyup.debounce.1500="
                        if(searchQuery.length > 2) {
                            searching = true;
                            fetch(`<?php echo e(route('domains.search')); ?>?q=${encodeURIComponent(searchQuery)}`)
                                .then(r => r.json())
                                .then(data => {
                        ...
                        <div class="flex items-center justify-between p-4 rounded-lg border" :class="result.available ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'">
                            ...
                            <div class="text-right">
                                ...
                                <template x-if="result.available">
                                    <form action="<?php echo e(route('customer.cart.add')); ?>" method="POST" class="mt-2">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/zumi/php/road-map/talksasa-cloud/resources/views/customer/domains/index.blade.php ENDPATH**/ ?>