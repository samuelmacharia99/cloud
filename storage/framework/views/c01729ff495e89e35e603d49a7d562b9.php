<?php $__env->startSection('title', 'Domain Pricing'); ?>
...
<div x-data="pricingManager()" x-init="<?php if($errors->any()): ?> showAddExtensionModal = true <?php endif; ?>" class="space-y-6">
    <!-- Header -->
            ...
            <div x-data="{ expanded: false }" class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <button
                    ...
                    <div class="flex items-center gap-4 flex-1 text-left">
                        <div>
                            <h2 class="text-lg font-bold text-slate-900 dark:text-white"><?php echo e($extension->extension); ?></h2>
                            <p class="text-sm text-slate-600 dark:text-slate-400 mt-0.5"><?php echo e($extension->description ?? 'No description'); ?></p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mr-4">
                        <span class="px-3 py-1 bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white rounded-full text-xs font-medium">
                            <?php echo e($extension->registrar); ?>

                        </span>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo e($extension->enabled ? 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300' : 'bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300'); ?>">
                            <?php echo e($extension->enabled ? '●' : '○'); ?>
                ...
                <div
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                            <?php $__currentLoopData = [1, 2, 3, 5, 10]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $period): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    ...
                                    <td class="px-6 py-3 text-sm">
                                        <?php if($retail): ?>
                                            <div class="font-bold text-slate-900 dark:text-white">$<?php echo e(number_format($retail->price, 2)); ?></div>
                                            <?php if($retail->setup_fee > 0): ?>
                                                <span class="text-xs text-slate-600 dark:text-slate-400">+$<?php echo e(number_format($retail->setup_fee, 2)); ?> setup</span>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td class="px-6 py-3 text-sm">
                                        <?php if($wholesale): ?>
                                            <div class="font-bold text-slate-900 dark:text-white">$<?php echo e(number_format($wholesale->price, 2)); ?></div>
                                            <?php if($wholesale->setup_fee > 0): ?>
                                                <span class="text-xs text-slate-600 dark:text-slate-400">+$<?php echo e(number_format($wholesale->setup_fee, 2)); ?> setup</span>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <span class="text-slate-500 dark:text-slate-400">—</span>
                                        <?php endif; ?>
                                    ...
                                    <td class="px-6 py-3 text-sm">
                                        <?php if($retail && $wholesale): ?>
                                            <div class="font-bold text-emerald-600 dark:text-emerald-400">$<?php echo e(number_format($retail->price - $wholesale->price, 2)); ?></div>
                                            <span class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(round((($retail->price - $wholesale->price) / $wholesale->price) * 100, 1)); ?>%</span>
                                        <?php else: ?>
                                            <span class="text-slate-500 dark:text-slate-400">—</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-6 py-3 text-center">
                                        <button @click="openPricingModal(<?php echo e($extension->id); ?>, <?php echo e($period); ?>, <?php echo e(json_encode(['retail' => $retail, 'wholesale' => $wholesale])); ?>)" type="button" class="px-3 py-1 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition bg-blue-50 dark:bg-blue-950 rounded">
                                            Edit
                                        </button>
                                    </td>
                                </tr>
    ...
    <div x-show="showAddExtensionModal" @click.outside="closeAddExtensionModal()" x-transition class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 max-w-md w-full mx-4 p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="<?php echo e(route('admin.domain-extensions.store')); ?>" class="space-y-4">
                <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <input
                        ...
                        required
                        class="w-full px-4 py-2 border <?php echo e($errors->has('extension') ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'); ?> bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-sm">
...
$message = $__bag->first($__errorArgs[0]); ?><p class="text-xs text-red-600 dark:text-red-400 mt-1"><?php echo e($message); ?></p><?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    ...
                    <input
                        ...
                        value="<?php echo e(old('registrar')); ?>"
                        required
                        class="w-full px-4 py-2 border <?php echo e($errors->has('registrar') ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'); ?> bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-sm">
...
unset($__errorArgs, $__bag); ?>
    ...
    <div x-show="showPricingModal" @click.outside="showPricingModal = false" x-transition class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 max-w-xl w-full mx-4 p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="<?php echo e(route('admin.domains.pricing.store')); ?>" class="space-y-4">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/zumi/php/road-map/talksasa-cloud/resources/views/admin/domains/pricing.blade.php ENDPATH**/ ?>