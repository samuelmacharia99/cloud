    ...
    <?php if($services->count() > 0): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('customer.services.show', $service)); ?>" class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 hover:border-blue-300 dark:hover:border-blue-700 transition-colors">
                    <!-- Header -->
                    <div class="mb-4">
                        <div class="flex items-start justify-between mb-2">
                            <h3 class="text-lg font-semibold text-slate-900 dark:text-white"><?php echo e($service->product->name); ?></h3>
                        </div>
                    ...
                    <div class="space-y-3 mb-6 pb-6 border-b border-slate-200 dark:border-slate-800">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-medium text-slate-900 dark:text-white">#<?php echo e($service->id); ?></span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-xs text-slate-600 dark:text-slate-400">Billing Cycle</span>
                            <span class="text-sm font-medium text-slate-900 dark:text-white"><?php echo e(ucfirst($service->billing_cycle)); ?></span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-xs text-slate-600 dark:text-slate-400">Next Due</span>
                            <span class="text-sm font-medium text-slate-900 dark:text-white"><?php echo e($service->next_due_date?->format('M d, Y') ?? '-'); ?></span>
                        </div>
                    </div>
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/zumi/php/road-map/talksasa-cloud/resources/views/customer/services/index.blade.php ENDPATH**/ ?>