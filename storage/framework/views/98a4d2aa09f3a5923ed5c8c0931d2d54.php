<?php $__env->startSection('title', 'Cron Jobs'); ?>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2"><?php echo e($stats['total_jobs']); ?></p>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2"><?php echo e($stats['enabled_jobs']); ?></p>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2"><?php echo e($stats['total_runs_24h']); ?></p>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold <?php echo e($stats['failed_runs_24h'] > 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-white'); ?> mt-2">
                            <?php echo e($stats['failed_runs_24h']); ?>

                            <?php if($stats['failed_runs_24h'] > 0): ?>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        <?php $__empty_1 = true; $__currentLoopData = $jobs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $job): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
                                <td class="px-6 py-4">
                                    <a href="<?php echo e(route('admin.cron.show', $job)); ?>" class="text-blue-600 dark:text-blue-400 hover:underline font-medium">
                                        <?php echo e($job->name); ?>

                                    </a>
                                    <?php if($job->description): ?>
                                        <p class="text-xs text-slate-600 dark:text-slate-400 mt-1"><?php echo e($job->description); ?></p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4">
                                    <code class="bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-2 py-1 rounded text-xs font-mono"><?php echo e($job->schedule); ?></code>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <?php if($job->latestLog): ?>
                                        <div class="flex items-center gap-2">
                                            <?php if($job->latestLog->status === 'success'): ?>
                                                <span class="w-2 h-2 bg-green-500 rounded-full"></span>
                                                <span class="text-slate-900 dark:text-slate-100"><?php echo e($job->latestLog->started_at->diffForHumans()); ?></span>
                                            <?php elseif($job->latestLog->status === 'failed'): ?>
                                                <span class="w-2 h-2 bg-red-500 rounded-full"></span>
                                                <span class="text-red-600 dark:text-red-400"><?php echo e($job->latestLog->started_at->diffForHumans()); ?></span>
                                            <?php else: ?>
                                ...
                                <td class="px-6 py-4">
                                    <?php if($job->next_run_at): ?>
                                        <span class="text-slate-900 dark:text-slate-100"><?php echo e($job->next_run_at->diffForHumans()); ?></span>
                                    <?php else: ?>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <button onclick="runJob(<?php echo e($job->id); ?>, '<?php echo e(addslashes($job->name)); ?>')" class="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800 rounded text-xs font-medium transition-all" title="Run now">
                                        ...
                                        <form action="<?php echo e(route('admin.cron.toggle', $job)); ?>" method="POST" class="inline">
                                            <?php echo csrf_field(); ?>
                                            <button type="submit" class="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-xs font-medium transition-all" title="<?php echo e($job->enabled ? 'Disable' : 'Enable'); ?>">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                                                </svg>
                                            </button>
                                        </form>
                                        <a href="<?php echo e(route('admin.cron.show', $job)); ?>" class="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-xs font-medium transition-all" title="View details">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    setInterval(function() {
        fetch('<?php echo e(route("admin.cron.logs", ["job" => "system"])); ?>'.replace('system', 'system'))
            .catch(e => {});
    }, 15000);
</script>
<?php $__env->stopPush(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/zumi/php/road-map/talksasa-cloud/resources/views/admin/cron/index.blade.php ENDPATH**/ ?>