<?php $__env->startSection('title', 'Service: ' . $service->product->name); ?>
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8" x-data="{ tab: 'overview' }">
        <div class="flex items-start justify-between mb-6">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 dark:text-white"><?php echo e($service->product->name); ?></h1>
                <p class="text-slate-600 dark:text-slate-400 mt-2">Service #<?php echo e($service->id); ?> • <?php echo e(ucfirst(str_replace('_', ' ', $service->product->type))); ?></p>
        ...
        <div class="mt-6">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="text-slate-900 dark:text-white font-medium">#<?php echo e($service->id); ?></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-600 dark:text-slate-400">Status</span>
                                <span class="text-slate-900 dark:text-white font-medium"><?php echo e(ucfirst($service->status)); ?></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-600 dark:text-slate-400">Created</span>
                                <span class="text-slate-900 dark:text-white font-medium"><?php echo e($service->created_at->format('M d, Y')); ?></span>
                            </div>
                            <?php if($service->terminate_date): ?>
                                <div class="flex justify-between">
                                    <span class="text-slate-600 dark:text-slate-400">Terminated</span>
                                    <span class="text-slate-900 dark:text-white font-medium"><?php echo e($service->terminate_date->format('M d, Y')); ?></span>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="text-slate-900 dark:text-white font-medium"><?php echo e(ucfirst($service->billing_cycle)); ?></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-600 dark:text-slate-400">Next Due</span>
                                <span class="text-slate-900 dark:text-white font-medium"><?php echo e($service->next_due_date?->format('M d, Y') ?? '-'); ?></span>
                            </div>
                            <?php if($service->invoice): ?>
                                <div class="flex justify-between">
                                    <span class="text-slate-600 dark:text-slate-400">Invoice</span>
                                    <a href="<?php echo e(route('customer.invoices.index')); ?>" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium">#<?php echo e(str_pad($service->invoice->id, 5, '0', STR_PAD_LEFT)); ?></a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div x-show="tab === 'billing'" class="space-y-4">
                <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                    ...
                    <div class="space-y-3 text-sm">
                        <?php if($service->product->monthly_price): ?>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-600 dark:text-slate-400">Monthly Price</span>
                                <span class="text-slate-900 dark:text-white font-medium">$<?php echo e(number_format($service->product->monthly_price, 2)); ?></span>
                            </div>
                        <?php endif; ?>
                        <?php if($service->product->yearly_price): ?>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-600 dark:text-slate-400">Yearly Price</span>
                                <span class="text-slate-900 dark:text-white font-medium">$<?php echo e(number_format($service->product->yearly_price, 2)); ?></span>
                            </div>
                        <?php endif; ?>
                        <div class="flex justify-between items-center pt-3 border-t border-slate-200 dark:border-slate-700">
                            ...
                            <span class="text-slate-900 dark:text-white font-semibold"><?php echo e(ucfirst($service->billing_cycle)); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <?php if($service->invoice): ?>
                    <div class="bg-blue-50 dark:bg-blue-950 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                        <p class="text-sm text-blue-900 dark:text-blue-100">
                            <strong>Invoice <?php echo e(str_pad($service->invoice->id, 5, '0', STR_PAD_LEFT)); ?>:</strong> $<?php echo e(number_format($service->invoice->total, 2)); ?> • Status: <?php echo e(ucfirst($service->invoice->status)); ?>

                        </p>
                        <a href="<?php echo e(route('customer.invoices.index')); ?>" class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 mt-2 inline-block">
                            View Invoice →
                        </a>
                    </div>
                <?php endif; ?>
            ...
            <div x-show="tab === 'credentials'" class="space-y-4">
                ...
                <?php if($service->status === 'active' && $service->credentials): ?>
                    <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                        ...
                        <div class="bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700 p-3">
                            <p class="text-xs font-mono text-slate-600 dark:text-slate-400 break-all"><?php echo e($service->credentials); ?></p>
                        </div>
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/zumi/php/road-map/talksasa-cloud/resources/views/customer/services/show.blade.php ENDPATH**/ ?>